...
// ── Definido FUERA del componente principal para evitar remount en cada render ──
    ...
    <div>
      ...
      <input
        ...
        onBlur={() => {
          ...
          const display = num.toLocaleString('es-AR', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })
          setData({ ...data, amountDisplay: display })
        }}
        onFocus={() => {
          setData({ ...data, amountDisplay: data.amount ? String(data.amount) : '' })
        }}
...
function PaymentForm({ data, setData, onSubmit, onCancel, submitLabel, properties }) {
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-xs text-stone-500 block mb-1">Fecha de pago</label>
...
export default function Payments({ session }) {
  ...
  const startEdit = () => {
    ...
    const display = num
      ? parseFloat(num).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
      : ''
    setEditForm({
      property_id:    selectedPayment.property_id    || '',
      type:           selectedPayment.type           || 'alquiler',
      period:         selectedPayment.period         || '',
      amount:         String(num),
      amountDisplay:  display,
      payment_date:   selectedPayment.payment_date   || '',
      payment_method: selectedPayment.payment_method || 'transferencia',
      status:         selectedPayment.status         || 'pagado',
    ...
    return (
      <MainLayout session={session} role={profile?.role}>
        <div className="p-8">
            ...
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
              <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4 shadow-xl">
                ...
                <p className="text-sm text-stone-500 mb-6">
                  Estas a punto de modificar los datos de este pago. Queres continuar?
                </p>
                <div className="flex gap-3">
              ...
              <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4 shadow-xl">
                ...
                <p className="text-sm text-stone-500 mb-6">
                  ...
                  <span className="font-semibold text-stone-800">
                    {selectedPayment.type} — {selectedPayment.properties?.name}
                  </span>.
          ...
          <button
            onClick={() => { setSelectedPayment(null); setEditMode(false); setEditForm(null) }}
          ...
          <div className="bg-blue-50 rounded-2xl px-8 py-6 mb-6 flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-blue-900 capitalize">
                {selectedPayment.type} — {selectedPayment.properties?.name}
              </h1>
              <p className="text-sm text-blue-600 mt-1">
                {selectedPayment.period && `${selectedPayment.period} · `}
        ...
        <div className="bg-white rounded-xl border border-stone-200">
          ...
          {payments.length === 0 ? (
            <div className="px-6 py-12 text-center">
              <p className="text-sm text-stone-400">No hay pagos registrados todavia</p>
            </div>
          ) : (
            payments.map((payment, i) => {
              ...
              return (
                <div key={payment.id}
                  ...
                  className={`flex items-center gap-4 px-6 py-4 cursor-pointer hover:bg-stone-50 transition-colors ${
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-stone-800 capitalize">
                      {payment.type} — {payment.properties?.name}
                    </p>
